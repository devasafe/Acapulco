...
import {
  listControlled, getState, configure, jump, setTarget, setTrend, preset,
...
export default function MarketControlPage() {
  ...
  const loadList = useCallback(async () => {
    try {
      const { data } = await listControlled();
      setAssets(data);
    ...
    >
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {assets.map((a) => (
          <button
            key={a._id}
            onClick={() => { setSelected(a._id); setState(null); }}
        ...
        {assets.length === 0 && (
          <p className="text-on-surface-variant text-body-sm">
            Nenhum ativo controlado. Configure um ativo (priceMode=controlled) abaixo após selecioná-lo no admin de ativos.
          </p>
        )}
      </div>
